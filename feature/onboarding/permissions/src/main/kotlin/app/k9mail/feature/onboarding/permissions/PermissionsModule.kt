...
import app.k9mail.feature.onboarding.permissions.domain.usecase.SaveBlockPermission
...
val featureOnboardingPermissionsModule: Module = module {
    ...
    factory<UseCase.IncreaseDenyAndCheckIfBlock> { SaveBlockPermission(permissionChecker = get()) }
    factory<UseCase.HasRuntimePermissions> { HasRuntimePermissions(permissionsModelChecker = get()) }

    viewModel {
        PermissionsViewModel(
            checkPermission = get(),
            increaseDenyAndCheckIfBlock = get(),
        )
    }
}
